...
const ShopGrid = (props) => {
  ...
  useEffect(() => {
    ...
    // for header name
    const dataName = Parse.Object.extend('Category');
    ...
    //for fet product aginest subcategory
  ...
  const handleBrand = (dataid) => {
    setBrandId(dataid)
    const Products = Parse.Object.extend("Products");
    const query = new Parse.Query(Products);
    query.descending("createdAt");
    query.ascending('suggestedRetailPrice')
    query.equalTo("brands", { "__type": "Pointer", "className": "Brands", "objectId": dataid })
    query.equalTo("SubCategory", { "__type": "Pointer", "className": "Category", "objectId": id })
    query.equalTo('isVisible', true)
    query.include("image");
    query.limit(12);
    // query.matchesQuery("brands", query2);
    query.find().then(
      (products) => {
        for (let i = 0; i < products.length; i++) {
          let productId = products[i].id;
          ...
          <div className="row">
            <div className="col-main col-sm-9 col-xs-12 col-sm-push-3">
              <div class="banner_image">
                {headerName && headerName[0].get('category_image') ?
                  <img className='img-fluid' src={headerName && headerName[0].get('category_image')._url} />
                  : <img className='img-fluid' src='https://source.unsplash.com/WLUHO9A_xik/1000x600' />}
              </div>
                    ...
                    {sellingcard.length !== 0 ?
                      sellingcard && sellingcard.map((data, index) => {
                        if (data.get('name')) {
                          ...
                          return (
                            <li
                              className="item col-lg-4 col-md-4 col-sm-6 col-xs-6 " key={index}>
                              <div
              ...
              <div className="block shop-by-side">
                <div className="block-content">
                  ...
                  <div className="block-content">
                    <div className="slider-range">
                      ...
                      <ul className="check-box-list">
                        ...
                        <li>
                          <input type="checkbox" id="p7" name="cc" onClick={() => clearAllFilter(brandId)} />
                  ...
                  <div className="layered-Category">
                    <h2 className="saider-bar-title">Brands</h2>
